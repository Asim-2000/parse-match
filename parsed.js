...
test("sha1 node", function () {
  ...
  HASH_SAMPLES.forEach(function (sample) {
    ...
    assert.equal(
      hashToHex(Array.prototype.slice.apply(sha1(sample.input))),
      sample.sha1
    );
  });
});
...
test("md5 node", function () {
  ...
  HASH_SAMPLES.forEach(function (sample) {
    // Convert the sha1 Buffer to an Array here so we can call map() on it in hashToHex
    assert.equal(
      hashToHex(Array.prototype.slice.apply(md5(sample.input))),
      sample.md5
    );
  });
});
...
test("v3", function () {
  ...
  var testBuf = [
    0x91, 0x25, 0xa8, 0xdc, 0x52, 0xee, 0x36, 0x5b, 0xa5, 0xaa, 0x81, 0xb0,
    0xb3, 0x68, 0x1c, 0xf6,
  ];
...
test("v5", function () {
  ...
  var buf = new Array(16);
  var testBuf = [
    0xfd, 0xda, 0x76, 0x5f, 0xfc, 0x57, 0x56, 0x04, 0xa2, 0x69, 0x52, 0xa7,
    0xdf, 0x81, 0x64, 0xec,
  ];
